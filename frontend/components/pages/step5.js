...
export default function Analytics() {
  ...
  useEffect(() => {
    console.log(
      "tipsAverage: ",
      ((parseFloat(tips.replace(",", ".")) / revenue) * 100).toFixed(2)
    );
    if (shouldUpdateTips) {
      setTipsAverage(
        ((parseFloat(tips.replace(",", ".")) / revenue) * 100).toFixed(2)
  ...
  return (
    <>
      <div>
        ...
        <p className="mb-4">
          Wow, was ein toller Basar! Hier siehst du noch einmal schwarz auf
          weiß, wie gut er gelaufen ist - vielleicht kannst du aus den
          Informationen fürs nächste Mal lernen.
        </p>
        <div className="grid grid-cols-3 gap-6">
          <Card title={`${animatedRevenue}€`} description="Umsatz" />
          <Card
            title={`${animatedProfit}€`}
            description={`Profit bei aktueller Provisionsrate (${provision}%)`}
          />
          ...
          <div className="col-span-2 row-span-2 rounded-md border border-ourLightGray bg-white p-4 shadow-md">
            <Graph
              ...
              unsoldOffers={unsoldOffers.length}
              reclinedOffers={reclinedOffers.length}
            />
          </div>
          <Card
            title={
            ...
            info="Das Trinkgeld musst du manuell eintragen"
          />
